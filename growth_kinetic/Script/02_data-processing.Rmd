...
load(file.path(output_dir, "output1.RData"))
```
...
ggplot(data, aes(x = time, y = OD, color = fct_reorder2(strain, time, OD))) +
          geom_point(cex=0.7) + labs(color = "Strain")
```
...
ggplot(medium, aes(x = time, y = OD, color = mol_conc, shape = replicate)) +
          geom_point(cex=1) + labs(color = "3-HP concentration (mM)")

cat("Mean medium OD is", mean(medium$OD))
```
...
ggplot(no_medium, aes(x = time, y = OD, color = fct_reorder2(strain, time, OD))) + geom_point(cex=1) + labs(color = "Inoculum")
...
```{r}
# Print the conditions where the OD surpassed 0.25 at some point (with other words, where the OD surpassed 0.25)
MG1655 %>%
  filter(OD >= .25) %>%

...
reshape_df <- function(input_df) {
  #' Reshape a 2x3 dataframe to a 1x6 dataframe
  #'
  #' This function takes a 2x3 dataframe containing OD values and reshapes it
  #'   into a 1x6 dataframe.
  #'
  #' @param input_df A 2x3 dataframe containing OD values.
  #' @return A tibble with two columns: `OD` containing the OD values, and
  #'   `well` containing the corresponding well numbers.
  #'
  #' @examples
  #' \dontrun{
  #' input_df <- data.frame(
...
parse_6well_plates <- function(file_path_to_data, num_experiments, timepoints) {
  #' Parse multiple readings from 6-well plates
  #'
  ...
  #'   increasing time points (e.g., Exp 1 (timepoint 1), Exp 2 (timepoint 1),
  #'   Exp 3 (timepoint 1)), and so on.
  #' * The blank OD measurements are subtracted from each measurement with
  #'   corresponding experiment ID and well number.
  #'
  #' @param file_path_to_data A character string containing the file path to
  #'   your raw data .xlsx file.
  #' @param num_experiments The number of independent plates, each containing a
  #'   separate experiment. Must equal 1, 2, or 3.
  #' @param timepoints A vector of timepoints corresponding to the readings.
  #' @return A long tibble with parsed data from the 6-well plates, including
  #'   (raw) OD value, well number, experiment ID, time (in hours), and the
  #'   `ODc` column containing the OD values corrected for the blank.
  #'
  #' @examples
  #' \dontrun{
  ...
  if (!num_experiments %in% c(1, 2, 3)) {
    cli::cli_abort(c("{.var num_experiments} must equal 1, 2, or 3.",
      "x" = "You've tried to set {.var num_experiments} to {num_experiments}."
    ))
  }
  ...
  if (length(timepoints) * num_experiments != n_datapoints) {
    cli::cli_abort(c(
      ...
      "x" = "{.var timepoints} has length {length(timepoints)}.",
      "x" = "You have {n_datapoints} data points (excluding blanks).",
      "i" = "Review your input of {.var num_experiments} and {.var timepoints}."
    ))
  }
...
output <- parse_6well_plates(
  file.path(data_dir, paste(name, "xlsx", sep = ".")), # Normally, you don't have to change this
  num_experiments = 2,
  timepoints = 0:12 # A vector that contains the timepoints for each measurement. This is manually determined by the user based on their experiment.
)
```
...
fwrite(output, file.path(output_dir, paste(name, "processed.csv", sep = "_")))
```
